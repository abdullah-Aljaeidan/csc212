public class LinkedlistADT {
    ...
    public void delete() { // we want to delete a contact that is simple, we would create a method in list
                           // all events (delete based on number or name) that method recives name or
                           // number based from the contact that we want to delete and we itrate through
                           // allevents and and accsess each "event object" and accsess each contact
                           // attribute name then if true delete
        if (current == head) {
            head = head.getNext();
        } else {
            Node tmp = head;
